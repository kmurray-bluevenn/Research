...
module D3 {
	...
	//	min: (arr: number[], map?: (v: any) => any) => number;
		...
		*/
		max: {
			(arr: any[], map?: (v: any) => any): number;
			(arr: number[], map?: (v: any) => any): number;
		};
		/**
	...
	interface Selection extends Selectors {
		...
		call(callback: (selection: Selection, x:any) => void ): Selection;
	...
	interface Transition {
		...
		attrTween: (name: string, data: (data: any) => any) => Transition;
		ease: (t: string) => Transition;
		remove: () => Selection;
	}
	...
	interface Axis {
		...
		tickFormat(formatter: (value: any) => any): Axis;
	}
		...
		*/
		x: {
			...
			*/
			...
			(accessor: (data: any, index: number) => any): Line;
		};
		/**
		...
		*/
		x: {
			...
			*/
			...
			(accessor: (data: any, index:number) => any): Area;
		};
		/**
